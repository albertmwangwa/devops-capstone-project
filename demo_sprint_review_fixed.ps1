Write-Host "=== SPRINT REVIEW DEMO (FIXED) ===" -ForegroundColor Cyan
...
# Test if service is running using Invoke-WebRequest (PowerShell native)
...
try {
    ...
    Write-Host "   ✗ Service not responding or not running" -ForegroundColor Red
...
try {
    $existing = Invoke-RestMethod -Uri "http://localhost:5000/accounts" -TimeoutSec 3
    if ($existing -and @($existing).Count -gt 0) {
        foreach ($account in $existing) {
            Write-Host "   Deleting account ID: $($account.id)" -ForegroundColor Gray
    ...
    Write-Host "   No accounts exist or error: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
# Use curl.exe for the actual curl command
$createOutput = curl.exe -i -X POST http://127.0.0.1:5000/accounts -H "Content-Type: application/json" -d '{"name":"John Doe","email":"john@doe.com","address":"123 Main St.","phone_number":"555-1212"}'
...
Write-Host "`n   Output:" -ForegroundColor Gray
$listOutput = curl.exe -i -X GET http://127.0.0.1:5000/accounts
...
Write-Host "`n   Output:" -ForegroundColor Gray
$readOutput = curl.exe -i -X GET http://127.0.0.1:5000/accounts/1
...
Write-Host "`n   Output:" -ForegroundColor Gray
$updateOutput = curl.exe -i -X PUT http://127.0.0.1:5000/accounts/1 -H "Content-Type: application/json" -d '{"name":"John Doe","email":"john@doe.com","address":"123 Main St.","phone_number":"555-1111"}'
...
Write-Host "`n   Output:" -ForegroundColor Gray
$deleteOutput = curl.exe -i -X DELETE http://127.0.0.1:5000/accounts/1
...
try {
    $finalList = Invoke-RestMethod -Uri "http://localhost:5000/accounts" -TimeoutSec 3
    ...
    Write-Host "   Error checking accounts: $($_.Exception.Message)" -ForegroundColor Red
}
